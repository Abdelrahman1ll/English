...
export function FamilyPage() {
  ...
  const rawData = useMemo(() => (levelData?.vocabulary?.FAMILY as readonly VocabularyItem[]) || [], [levelData]);

  const studyItems: StudyItem[] = useMemo(() => rawData.map(i => ({ primary: i.english || "", secondary: i.arabic || "", category: "Family Members" })), [rawData]);
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-16 animate-in fade-in duration-500 pb-20">
      <header className="border-b border-white/5 pb-8">
        <h1 className="text-4xl font-black text-white tracking-tight flex items-center gap-4"><User className="text-blue-400" size={32} /> Family Members</h1>
        <p className="text-neutral-400 mt-3 text-lg">Learn the names of family members and relations.</p>
      </header>
      ...
      {isStudyMode ? <StudyModule items={studyItems} onExit={() => setIsStudyMode(false)} /> : (
        <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredData.map((item, idx) => <FamilyMemberCard key={item.english} item={item} index={idx} isActive={activeWord === item.english} isPlaying={playingItem === item.english} onClick={handleCardClick} />)}
          </div>
        </>
      )}
      <InstructionCard />
