...
export function GreetingsBasicsPage() {
  ...
  const filteredSentences = useMemo(() => {
    const SENTENCES_DATA =
      (levelData?.sentences as { SENTENCES_DATA?: readonly SentenceItem[] })
        ?.SENTENCES_DATA || [];

    const NUMBERS_DATA = (levelData?.vocabulary?.NUMBERS_DATA as any) || {
      BASICS: [],
    };
    const numberSentences: SentenceItem[] = NUMBERS_DATA.BASICS.map(
      (n: any) => ({
        english: n.word,
        arabic: n.digit.toString(),
  ...
  const groupedSentences = useMemo(() => {
    const groups: Record<
      string,
      { icon: LucideIcon | undefined; items: SentenceItem[] }
    > = {};
