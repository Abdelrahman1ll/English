...
export function IntroduceMyselfPage() {
  ...
  const filteredSentences = useMemo(() => {
    const categories = ["Introduce Myself"];
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-12 animate-in fade-in duration-500 pb-20">
      <header className="border-b border-white/5 pb-6">
        <h1 className="text-4xl font-black text-white tracking-tight flex items-center gap-4"><div className="p-3 bg-blue-500/10 rounded-2xl"><Sparkles className="text-blue-400" size={32} /></div> Let me introduce myself</h1>
        <p className="text-neutral-400 mt-4 text-lg">Learn how to talk about yourself, your family, your hobbies, and favorites.</p>
      </header>
      ...
      {isStudyMode ? <StudyModule items={studyItems} onExit={() => setIsStudyMode(false)} /> : (
        <>
          <div className="relative group max-w-2xl">
            ...
            <input type="text" placeholder="Search sentences..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full bg-[#1a1a1a] border border-white/10 rounded-3xl py-5 pl-14 pr-6 text-white text-lg focus:ring-2 focus:ring-blue-500/50 outline-hidden font-arabic" />
          </div>

          <div className="space-y-16">
            {Object.entries(groupedSentences).map(([category, { items }]) => (
