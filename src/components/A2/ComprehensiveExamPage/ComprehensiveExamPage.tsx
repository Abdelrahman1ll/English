...
import { validateWriting } from "./WritingSection/utils";
...
export const ComprehensiveExamPage: React.FC = () => {
      ...
      page.sections.forEach((section) => {
        section.questions.forEach((q) => {
          ...
          if (q.type === "fill-in") {
            ...
            const writingData = userAnswer || { text: "", errorCount: 0 };
            const { score: writingScore } = validateWriting(
              writingData.text || "",
              writingData.errorCount || 0,
            );
            totalScore += writingScore;
            isCorrect = writingScore >= 3; // For 'correct' status in review
          } else {
  ...
  if (isSubmitted) {
            ...
            <div className="space-y-6">
              {wrongQuestions.length === 0 ? (
                ...
                wrongQuestions.map((q, idx) => (
                  <div
                    key={q.id}
                    className="bg-slate-50 dark:bg-neutral-900 p-8 rounded-[2rem] border border-slate-200 dark:border-white/5 relative overflow-hidden"
                  >
    ...
    return (
      <div className="min-h-screen bg-white dark:bg-black py-8 px-4 flex items-center justify-center">
        <div className="max-w-md w-full bg-slate-50 dark:bg-neutral-900 rounded-[2rem] shadow-2xl overflow-hidden border border-slate-200 dark:border-white/5">
          ...
          <div className="p-8 text-center">
            ...
            <div className="flex flex-col gap-3">
              ...
              <button
                ...
                className="w-full py-4 bg-violet-600 hover:bg-violet-700 text-white font-black rounded-2xl transition-all shadow-xl shadow-violet-500/20 active:scale-95"
              >
                RETAKE EXAM
              </button>
              <button
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-black py-6 px-4 md:py-12 md:px-8">
      <div className="max-w-5xl mx-auto">
        {/* Header */}
        <div className="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div className="space-y-1">
            <div className="flex items-center gap-3">
              <span className="bg-violet-600 text-white px-2.5 py-0.5 rounded-lg text-xs font-black tracking-widest">
                A2
              </span>
          ...
          <div className="flex flex-col items-end gap-3">
            ...
            <p className="text-xs font-black text-slate-400 dark:text-neutral-500 uppercase tracking-widest">
              Page <span className="text-violet-600">{currentPageIndex + 1}</span>{" "}
              of {examPages.length}
            </p>
          </div>
        </div>
        ...
        {/* Page Container */}
        <div className="bg-slate-50 dark:bg-slate-900 rounded-[2.5rem] shadow-3xl border border-slate-200 dark:border-white/5 overflow-hidden relative">
            ...
            {currentPage.sections.map((section, sIdx) => (
              <div key={sIdx} className={`${sIdx > 0 ? "mt-16" : ""}`}>
                <div className="flex items-center gap-4 mb-10">
                  <h2 className="text-lg md:text-xl font-black text-violet-600 dark:text-violet-500 uppercase tracking-widest">
                    {section.title}
                  </h2>
                    ...
                    <div key={q.id} className="relative group">
                      <div className="flex flex-col gap-6 p-2">
                        <div className="flex gap-4">
                          <span className="flex-shrink-0 w-8 h-8 rounded-xl bg-violet-100 dark:bg-violet-500/10 text-violet-600 dark:text-violet-400 flex items-center justify-center text-xs font-black">
                            {qIdx + 1}
                          </span>
                            ...
                            {Array.isArray(q.correctAnswer) ? (
                              q.correctAnswer.map((_, idx) => (
                                <input
                                  ...
                                  type="text"
                                  placeholder={`___`}
                                  className="px-5 py-3 rounded-2xl border-2 border-slate-200 dark:border-neutral-800 bg-white dark:bg-black focus:border-violet-500 dark:focus:border-violet-500 focus:ring-4 focus:ring-violet-500/10 outline-none transition-all w-32 text-center font-black text-lg text-violet-600"
                              ...
                              <input
                                ...
                                className="px-5 py-3 rounded-2xl border-2 border-slate-200 dark:border-neutral-800 bg-white dark:bg-black focus:border-violet-500 dark:focus:border-violet-500 focus:ring-4 focus:ring-violet-500/10 outline-none transition-all w-64 text-center font-black text-lg text-violet-600"
                                value={answers[q.id] || ""}
                                onChange={(e) =>
                            ...
                            {q.options?.map((option) => (
                              <button
                                ...
                                className={`flex items-center justify-between p-4 px-6 rounded-2xl border-2 transition-all text-left ${
                                  answers[q.id] === option
                                    ? "border-violet-500 bg-violet-50 dark:bg-violet-500/10 text-violet-700 dark:text-violet-400 shadow-lg shadow-violet-500/10"
                                    : "border-slate-200 dark:border-neutral-800 bg-white dark:bg-black hover:border-violet-300 dark:hover:border-neutral-700 hover:bg-slate-50 dark:hover:bg-neutral-900"
                                }`}
                              >
                        ...
                        {q.type === "writing" && (
                          <div className="ml-12">
                            <WritingSection
                              ...
                              initialValue={answers[q.id]?.text || ""}
                            />
                          </div>
                        )}
                      </div>
          ...
          {/* Navigation Controls */}
          <div className="p-6 md:p-10 bg-slate-100 dark:bg-neutral-900/50 border-t border-slate-200 dark:border-white/5 flex justify-between items-center">
            <button
              onClick={prevSection}
              disabled={currentPageIndex === 0}
              className="flex items-center gap-2 px-6 py-3 text-slate-500 dark:text-neutral-500 font-black disabled:opacity-30 hover:text-violet-600 transition-all text-sm uppercase tracking-widest rounded-xl hover:bg-slate-200 dark:hover:bg-white/5"
            >
              <ChevronLeft size={20} />
              Back
            </button>
            ...
            {currentPageIndex < examPages.length - 1 ? (
              <button
                onClick={nextSection}
                className="flex items-center gap-3 px-10 py-3.5 bg-violet-600 text-white font-black rounded-2xl hover:bg-violet-700 shadow-2xl shadow-violet-500/20 transition-all active:scale-95 text-sm uppercase tracking-widest"
              >
                Next Page
    ...
    <span>
      {parts.map((part, i) => (
        <React.Fragment key={i}>
          {part}
          {i < parts.length - 1 && (
            <span className="inline-block w-12 border-b-4 border-violet-500/30 mx-2 align-bottom h-6"></span>
          )}
        </React.Fragment>
      ))}
    </span>
