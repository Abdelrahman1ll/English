...
import type { TimePhrase } from "./types";

interface TimePhraseCardProps {
  readonly item: TimePhrase;
...
export function TimePhraseCard({
  item,
  index,
  isActive,
  isPlaying,
  ...
  return (
    ...
    >
      ...
      <Volume2
        size={20}
        className={`transition-all ${isPlaying ? "text-amber-400 opacity-100 scale-125" : isActive ? "text-amber-400/60 opacity-100" : "text-neutral-700 opacity-0 group-hover:opacity-100"}`}
      />
    </button>
  );
}
