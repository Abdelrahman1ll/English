...
import { MessageCircle, Search, LayoutGrid, Focus } from "lucide-react";
...
export function CommonSocialPage() {
  ...
  const CATEGORIES = useMemo(() => ["Asking how things are going", "Long Time No See", "Surprise / Unexpected", "Breaking the Ice", "Health & Happiness (Positive)", "Status Updates (Busy / Neutral)", "Expressing Heavy Busyness", "Social", "Common", "Travel", "Expressing Misunderstanding", "Clarifying Yourself", "Ending Conversation Abruptly", "Good-byes", "Asking for Explanation"], []);

  const rawSentences = useMemo(() => ((levelData?.sentences as any)?.SENTENCES_DATA || []) as SentenceItem[], [levelData]);

  const filteredSentences = useMemo(() => rawSentences.filter(s => CATEGORIES.includes(s.category) && (s.english.toLowerCase().includes(searchQuery.toLowerCase()) || s.arabic.includes(searchQuery))), [rawSentences, searchQuery, CATEGORIES]);
  ...
  const studyItems: StudyItem[] = useMemo(() => filteredSentences.map(s => ({ primary: s.english, secondary: s.arabic, category: s.category })), [filteredSentences]);
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-12 animate-in fade-in duration-500 pb-20">
      <header className="border-b border-white/5 pb-6">
        <h1 className="text-4xl font-black text-white tracking-tight flex items-center gap-4"><div className="p-3 bg-emerald-500/10 rounded-2xl"><MessageCircle className="text-emerald-400" size={32} /></div> Daily Life & Social</h1>
        <p className="text-neutral-400 mt-4 text-lg">Phrases for social interactions and daily life.</p>
      </header>
      ...
      {isStudyMode ? <StudyModule items={studyItems} onExit={() => setIsStudyMode(false)} /> : (
        <>
          <div className="relative group max-w-2xl">
            ...
            <input type="text" placeholder="Search..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full bg-[#1a1a1a] border border-white/10 rounded-3xl py-5 pl-14 pr-6 text-white text-lg focus:ring-2 focus:ring-emerald-500/50 outline-hidden font-arabic" />
          </div>
            ...
            {Object.entries(groupedSentences).map(([category, { icon: Icon, items }]) => (
              <div key={category} className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {items.map((s, idx) => <SocialCard key={`${category}-${idx}`} sentence={s} index={filteredSentences.indexOf(s)} isActive={activeWord === s.english} isPlaying={playingItem === s.english} onClick={handleSpeak} />)}
                </div>
              </div>
            ))}
          </div>
