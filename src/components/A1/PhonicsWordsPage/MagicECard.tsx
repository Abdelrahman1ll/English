...
export function MagicECard({
  ...
  const renderLongWord = (word: string) => {
    if (!word.toLowerCase().endsWith('e')) return word;
    return (
      <>
        {word.substring(0, word.length - 1)}
        <span className="text-amber-500">{word.substring(word.length - 1)}</span>
      </>
    );
  };
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      >
        ...
        <span className="text-lg font-bold">{renderLongWord(pair.long)}</span>
        <Volume2
          size={12}
